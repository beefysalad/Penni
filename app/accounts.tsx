        ...
        <View className="gap-5 px-6 pt-6">
          <View className="rounded-[30px] border border-[#17211c] bg-[#0f1512] p-5">
            <View className="flex-row items-center justify-between gap-3">
              ...
              <Button className="h-12 rounded-full bg-[#8bff62] px-5" variant="ghost" size="sm">
            ...
            <View className="mt-5 rounded-[26px] bg-[#121916] p-4">
              ...
              <View className="mt-5 flex-row gap-2">
                {accountFilters.map((filter, index) => (
                  <View
                    key={filter}
                    className={`rounded-full px-4 py-2 ${index === 0 ? 'bg-[#8bff62]' : 'bg-[#1a221e]'}`}>
                    <Text
                      className={`text-sm font-semibold ${index === 0 ? 'text-[#07110a]' : 'text-[#7f8c86]'}`}>
                      {filter}
                    </Text>
                  </View>
                ))}
              ...
              <View className="mt-5 flex-row gap-3">
                <View className="flex-1 rounded-[22px] bg-[#0f1512] p-4">
                  ...
                  <View className="mt-4 flex-row items-end justify-between gap-2">
                    {weeklyBars.map((barClassName, index) => (
                      <View key={`${barClassName}-${index}`} className="flex-1 items-center gap-2">
                        <View className={`w-full rounded-full ${barClassName}`} />
            ...
            <View className="mt-5 flex-row flex-wrap gap-4">
              {accountCards.map((account) => (
                <View
                  key={account.name}
                  className={`w-[48%] rounded-[24px] p-4 ${account.accentClassName}`}>
                  <View className="flex-row items-start justify-between gap-2">
                    <View className={`size-11 items-center justify-center rounded-full ${account.iconClassName}`}>
                      <Icon as={CreditCardIcon} className="size-5 text-[#8bff62]" />
                    </View>
                    <Icon as={EllipsisIcon} className="size-4 text-[#6d786f]" />
                  </View>
                  <Text className="mt-5 text-base font-semibold text-[#f4f7f5]">{account.name}</Text>
                  <Text className="mt-1 text-sm text-[#7f8c86]">{account.type}</Text>
                  <Text className="mt-5 text-xs font-semibold uppercase tracking-[1.6px] text-[#6d786f]">
                    Balance
                  </Text>
            ...
            <View className="mt-5 gap-4">
              {budgets.slice(0, 2).map((budget) => (
                <View key={budget.name} className="gap-2">
                  ...
                  <View className={`h-3 overflow-hidden rounded-full ${budget.trackClassName}`}>
                    <View className={`h-full rounded-full ${budget.fillClassName}`} />
                  </View>
                </View>
              ))}
            </View>
