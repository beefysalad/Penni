...
export function ProfileAvatarButton({ className }: ProfileAvatarButtonProps) {
  ...
  return (
    <Pressable
      className={className ?? 'size-9 rounded-full'}
      onPress={() => router.push('/settings')}
      accessibilityRole="button"
      accessibilityLabel="Open profile and settings">
      <Avatar
        alt={`${userName}'s avatar`}
        className="size-9 rounded-full border border-[#8bff62]/25">
        <AvatarImage source={imageSource} />
        <AvatarFallback className="bg-[#131d17]">
          <Text className="text-xs font-semibold text-[#8bff62]">{initials}</Text>
        </AvatarFallback>
      </Avatar>
    </Pressable>
  );
